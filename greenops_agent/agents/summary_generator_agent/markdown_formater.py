...
def convert_to_google_docs(content_markdown, debug=False):
    """
    Converts markdown content to Google Docs-style requests without actually creating or sending to Google Docs.
    Only parses and returns the batch of requests that would be sent.
    
    :param content_markdown: Markdown text input
    :param debug: Debug flag to print internal steps
    :return: Dictionary with two keys: 'text_requests', 'style_requests'
    """
    ...
    for chunk in chunks:
        ...
        if header_match:
            ...
            table_content_requests, table_style_requests, table_end_index = get_table_content_request(
                table_data, index + 1, debug=debug
            )
    ...
    return {
        "requests": text_requests + style_requests
    }